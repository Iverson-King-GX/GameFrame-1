...
export class handler {
    ...
    public init(cb: Function, host = null, ...args): void {
        this.cb = cb;
        this.host = host;
        this.args = args;
    }
...
export class LoaderMgr {
    ...
    static getInst(): LoaderMgr {
        if (!this.inst) {
            this.inst = new LoaderMgr();
        }
        return this.inst;
    }
     ...
     * 
     * @param {Array<string>} urls 资源路径
     * @param {handler} cb 回调
     * @param {Array<cc.Asset>} assetTypes 资源类型
    ...
    /*     
        ...
        dump()
        {
            ...
            cc.log(`---------------------LoaderMgr dump end, totalCount=${count}--------------------------`);
        } */
}
